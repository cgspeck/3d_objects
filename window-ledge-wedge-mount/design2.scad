...
difference() {
    ...
    translate([mount_x / 2, fixing_bolt_1_y, fixing_bolt_1_z]) rotate([actual_angle-180,0,0]) NutSlitAssembly(
        diameter=shield_bolt_size,
        length=50,
        nut_depth=shield_nut_depth
    );

    translate([mount_x / 2, fixing_bolt_2_y, fixing_bolt_2_z]) rotate([actual_angle-180,0,0]) NutSlitAssembly(
        diameter=shield_bolt_size,
        length=50,
        nut_depth=shield_nut_depth
    );
