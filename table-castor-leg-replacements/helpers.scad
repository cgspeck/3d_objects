...
module doughnut(height, outer_radius, inner_radius, fn=fn) {
    difference() {
        ...
        translate([0,0,-epsilon]) cylinder_outer(height + (epsilon*2), inner_radius, fn);
    }
}
...
module NutHoleAssembly(
    ...
    translate([
        0,
        0,
        nut_depth + nut_total_height - epsilon
    ]) {
...
module LocatedHole(thickness, outer_dia=3, inner_dia=1.5, inner=true) {
    if (inner) {
        doughnut(thickness, outer_dia / 2, inner_dia / 2);
    } else {
        cylinder_outer(thickness, outer_dia / 2);
    }
}
...
module D(dimensions) {
    ...
    r=dimensions.z / 2;
    cube(dimensions - [r,0,0]);
    translate([dimensions.x - r, dimensions.y, r]) 
        rotate([90,0,0]) 
        cylinder_outer(dimensions.y, r);
}
