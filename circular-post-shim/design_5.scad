...
difference() {
    ...
    translate([0, bracket_dim.x/2 + de_minimis, bracket_base_z / 2]) rotate([90,0,0]) cylinder_outer(bracket_thickness + 2 * de_minimis, 5.75 / 2);
}
...
translate([0,-3.41,0]) fixing_tab();

translate([-fixing_tab_thickness + bracket_thickness,-3.41,0]) mirror([1,0,0]) rotate([0,0,90]) fixing_tab();
