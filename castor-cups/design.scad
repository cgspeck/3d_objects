...
box_y=actual_castor_wheel_width + (2 * 2min_thickness);
...
box_tran=[
    0,
    0,
    -(box_z/2) - actual_castor_wheel_rad + holder_depth
];

wheel_translation=[
    0,
    actual_castor_wheel_width / 2,
    0
];
...
difference() {
    translate(box_tran) roundedBox(box_d, 4, true);
    translate(wheel_translation) rotate([90, 0, 0]) cylinder_outer(actual_castor_wheel_width, actual_castor_wheel_rad);
