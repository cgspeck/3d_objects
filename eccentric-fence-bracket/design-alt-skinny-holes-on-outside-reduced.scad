...
horizontal_post_trans=[
    (verticle_post_outer_dim.x - horizontal_post_outer_dim.x),
    verticle_post_outer_dim.y,
    verticle_post_outer_dim.z - horizontal_post_outer_dim.z - de_minimis
];


module ScrewHead(extra_through=0, rebate_screw_head=false) {
    cylinder_outer(ref_thickness + 2 * de_minimis + extra_through, screw_hole_rad);

    if (rebate_screw_head) {
        translate([0,0,-extra_through]) cylinder_outer(ref_thickness  / 2 + 1 * de_minimis + extra_through, screw_head_rad);
    }
}
...
module EccentricBracketV2() {
    difference() {
        union() {
            hull() {
                translate([
                    ...
                    horizontal_post_outer_dim.x,
                    verticle_post_outer_dim.y,
                    verticle_post_outer_dim.z
                ]);
        ...
        translate([
            horizontal_post_trans.x + ref_thickness,
